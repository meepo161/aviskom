...
object MVZModel : TestModel() {
    override val progressProperty: DoubleProperty = SimpleDoubleProperty().also {
        it.onChange {
            measuredDataDuring.time.value = "%.0f".format(abs(it * specifiedMVZTestTime))
        }
    }
    ...
    val storedDataAfter = MVZData(descriptor = SimpleStringProperty("Сохранённая"))
