...
class OverM : KSPADTest(view = OverMView::class, reportTemplate = "overm.xlsx") {
    ...
    override fun initVars() {
        ...
        testModel.specifiedOverMTestTime =
            PreFillModel.testTypeProp.value.fields["OVER_M_TIME"]?.value.toDoubleOrDefault(0.0)
...
//        CM.device<C2000>(CM.DeviceID.UZ92).setObjectUMax(maxU)
    ...
    private fun regulationTo(minPercent: Double, maxPercent: Double = minPercent, step: Double, wait: Long) {
        ...
        val initValue = testModel.measuredM
        val initPercent = 10
        val timeout = 30_000

        while (isRunning && (testModel.measuredM < min || testModel.measuredM > max)) {
            if ((abs(testModel.measuredM - initValue) / initValue) < initPercent / 100.0) {
...
//                CM.device<PR>(CM.DeviceID.DD2).off30To5AmperageStage()
    ...
    private fun waiting() {
        ...
        sleepWhileRun(testModel.specifiedOverMTestTime.toInt(), progressProperty = testModel.progressProperty)
    }
...
//        CM.device<PR>(CM.DeviceID.DD2).offOtherAmperageStages()
    ...
    override fun saveProtocol() {
        ...
        reportFields["TEMP_OVER_M"] = testModel.measuredData.T.value
