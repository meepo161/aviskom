...
//                            cause = "Застревание АРН"
    ...
    private fun regulateVoltage(latrDevice: AvemLatrController1) {
        ...
        while (isRunning && testModel.measuredU < testModel.specifiedU_HV_SG - 300) {
    ...
    private fun accurateRegulate(latrDevice: AvemLatrController1) {
        if (isRunning) {
            ...
            while (isRunning &&
                (testModel.measuredU <= testModel.specifiedU_HV_SG ||
                        testModel.measuredU >= testModel.specifiedU_HV_SG * 1.03f)
            ) {
                if (testModel.measuredU <= testModel.specifiedU_HV_SG) {
                    latrDevice.plusVoltage()
                    sleep(200)
                }
                if (testModel.measuredU >= testModel.specifiedU_HV_SG * 1.03f) {
    ...
    private fun waiting() {
        ...
        sleepWhileRun(testModel.specifiedT_HV_SG.toInt(), progressProperty = testModel.progressProperty)
    }
    ...
    override fun saveProtocol() {
        ...
        reportFields["U_MEAS_HV_SG"] = testModel.measuredData.U.value
        reportFields["I_MEAS_HV_SG"] = testModel.measuredData.I.value
        reportFields["TIME_MEAS_HV_SG"] = testModel.measuredData.time.value
        reportFields["RESULT_HV_SG"] = testModel.measuredData.result.value
